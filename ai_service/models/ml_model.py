...
def confirmed_records(records):
    ...
    for r in records:
        ...
        # dict support (AI service)
        if isinstance(r, dict):
            if (
                r.get("doctor_confirmed") is True
                and r.get("doctor_final_diagnosis")
                and r.get("symptoms")
            ):
                filtered.append(r)

        # Django object support (training)
        else:
            if (
                getattr(r, "doctor_confirmed", False)
                and getattr(r, "doctor_final_diagnosis", None)
                and getattr(r, "symptoms", None)
            ):
                filtered.append(r)
...
def train_model(records):
    data = confirmed_records(records)
...
def train_clinical_model(records):
    data = list(confirmed_records(records))
...
def build_faiss_index(records):
    records = list(confirmed_records(records))
